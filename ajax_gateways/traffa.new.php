<?php
	...
	{
		if($boxid != '') $exceptions .= (' AND l.id != ' . $boxid);
	}
	...
	$query .= ' WHERE l.id = u.userid AND uf.user = l.id AND l.username != "Borttagen" AND z.zip_code = u.zip_code' . $agemarginquery . $exceptions . $genderquery;
	...
	while($data = mysql_fetch_assoc($result))
	{
		...
		//Main card
		$o .= '<li id="' . $data['userid'] . '" class="userbox ' . ($data['gender'] == 'm' ? 'male' : ($data['gender'] == 'f' ? 'female' : 'female')) . ' birth' . $birthsplit[0] . ' ' . ' newbox" style="display: none;" onClick="displayuser(' . $data['userid'] . ', \'' . $data['gender'] . '\');">' . "\n";
		...
		//Popup information
		$o .= '<div class="displayuser" id="wnd_' . $data['userid'] . '">';
		$genders = array('m' => 'kille', 'f' => 'tjej');
		$o .= '<p><a href="/traffa/profile.php?user_id=' . $data['userid'] . '">' . $data['username'] . '</a>';
		...
		$o .= '<input type="hidden" name="recipient" value="' . $data['userid'] . '" />' . "\n";
		...
		$o .= '<a style="display: block; position: absolute; bottom: 10px; right: 10px;" href="/traffa/profile.php?user_id=' . $data['userid'] . '">Besök profil &raquo;</a>';
		$o .= '</div>' . "\n";
	}
