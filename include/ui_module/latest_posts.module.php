<?php
	...
	foreach($threads as $thread)
	{
		if(forum_category_access($thread['read_threads']))
		{
			...
			$options['output'] .= '<li><span class="ui_module_latest_posts_written">' . date('H:i', $thread['last_post_timestamp']) . '</span> <a title="' . $info . '" href="' . $thread['url'] . '"' . ($thread['read_threads'] == 'joshua' ? 'style="color: orange;"' : '') . '>' . $thread['title'] . '</a></li>' . "\n";
		}
	}
	$options['output'] .= '</ul>' . "\n";
